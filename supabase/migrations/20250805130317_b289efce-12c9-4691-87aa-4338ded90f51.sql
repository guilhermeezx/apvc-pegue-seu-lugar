-- Add missing columns to estacas if they don't exist
ALTER TABLE estacas ADD COLUMN IF NOT EXISTS nome_reservante TEXT;
ALTER TABLE estacas ADD COLUMN IF NOT EXISTS telefone TEXT;
...
-- Function to create tournament with types and stakes
...
-- Update RLS policies to allow public read access for active tournaments
DROP POLICY IF EXISTS "Allow public read access for active tournaments" ON torneios;
CREATE POLICY "Allow public read access for active tournaments" 
ON torneios FOR SELECT 
USING (ativo = true);

DROP POLICY IF EXISTS "Allow public read access for bird types" ON tipos_passaro;
CREATE POLICY "Allow public read access for bird types" 
ON tipos_passaro FOR SELECT 
USING (true);

DROP POLICY IF EXISTS "Allow public read access for stakes" ON estacas;
CREATE POLICY "Allow public read access for stakes" 
ON estacas FOR SELECT 
USING (true);

-- Allow public updates for stake reservations
DROP POLICY IF EXISTS "Allow public updates for reservations" ON estacas;
CREATE POLICY "Allow public updates for reservations" 
ON estacas FOR UPDATE 
USING (status = 'disponivel');

-- Grant execute permissions on functions
GRANT EXECUTE ON FUNCTION authenticate_admin(TEXT, TEXT) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION create_tournament_complete(TEXT, DATE, DECIMAL, JSON) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION reserve_stake(UUID, TEXT, TEXT) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION confirm_stake_payment(UUID) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION cancel_stake_reservation(UUID) TO anon, authenticated;
